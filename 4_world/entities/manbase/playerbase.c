modded class PlayerBase {
	...
	bool PVEZ_ShouldBePardonedOnDeath() {
		...
		else if (pvez_DamageRedistributor && pvez_DamageRedistributor.IsKilledByAnotherPlayer()) {
			PlayerBase killerPlayer = PlayerBase.Cast(pvez_DamageRedistributor.GetKillerEntity());
			if (killerPlayer && killerPlayer.pvez_PlayerStatus && !killerPlayer.pvez_PlayerStatus.GetIsLawbreaker()) {
				return true;
			}
		}
		return false;
