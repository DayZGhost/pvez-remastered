class PVEZ_PlayerStatus : Managed {
	...
	bool PVEZ_IsPvpAttackAllowed(EntityAI attacker) {
...
#endif
		// eAI / Expansion AI bots are PlayerBase but have no pvez_PlayerStatus (no identity).
		// Allow their damage to pass through normally — they follow their own AI rules.
		...
		if (GetIsLawbreaker())
			return true;
		// if both fighters are in PVP zone:
		else if (GetIsInPVP() && attackerPlayer.pvez_PlayerStatus.GetIsInPVP())
			return true;
		// if config allows PVP when at least one of them is in PVP zone:
		else if (g_Game.pvez_Config.DAMAGE.Allow_Damage_Between_PVP_and_PVE)
			return GetIsInPVP() || attackerPlayer.pvez_PlayerStatus.GetIsInPVP();
		// if config unlocks PVP for a lawbreaker player:
		else if (attackerPlayer.pvez_PlayerStatus.GetIsLawbreaker() && g_Game.pvez_Config.LAWBREAKERS_SYSTEM.Allow_Lawbreakers_To_Attack_Anywhere)
			return true;
		else
			return false;
	}
