class PVEZ_KillManager : Managed {
	...
	static void OnPlayerKilled(PlayerBase victim, EntityAI killer, EntityAI weapon, int weaponType, bool killedInLaw) {
		...
		if (killedInLaw) {
			if (g_Game.pvez_Bounties.Enabled) {
				PlayerBase killerBounty = PlayerBase.Cast(killer);
				if (victim.pvez_PlayerStatus && killerBounty && killerBounty.pvez_PlayerStatus) {
					if (victim.pvez_PlayerStatus.GetIsLawbreaker() && !killerBounty.pvez_PlayerStatus.GetIsLawbreaker()) {
						string victimName = PVEZ_StaticFunctions.GetEntityName(victim);
						killerBounty.pvez_BountiesSpawner.SpawnRewards(g_Game.pvez_Bounties.Items, victimName);
					}
				}
			}
			return;
	...
	static void RegisterMurder(EntityAI killerEntity, EntityAI victimEntity, EntityAI weapon) {
		...
		if (killer && killer.pvez_PlayerStatus) {
